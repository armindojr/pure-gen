/**
 *
 * @namespace pure.document
 */
function Document(pure) {
    const self = this;
    /**
     * cpf
     * Method to generate random valid document number for Brazillian citizen
     *
     * @method pure.document.cpf
     * @param {object} opts Options passed through object
     * @param {boolean= } [opts.format=false] Define that result is formated or not
     * @memberof pure.document
     * @returns {string} valid document number
     */
    self.cpf = (opts = {}) => {
        const def = opts || {};
        let doc = '';

        for (let i = 0; i < 9; i += 1) {
            doc += pure.random.number({ min: 0, max: 9 });
        }

        const mod = (str, size, val) => {
            let sum = 0;
            for (let i = 1; i <= size; i += 1) {
                const subst = str.substring(i - 1, i);
                sum += parseInt(subst, 10) * (val - i);
            }

            let digit = (sum * 10) % 11;

            if ((digit === 10) || (digit === 11)) {
                digit = 0;
            }

            return digit;
        };

        doc += mod(doc, 9, 11);
        doc += mod(doc, 10, 12);

        if (def.format === true) {
            doc = doc.replace(/^(\d{3})(\d{3})(\d{3})(\d{2})/, '$1.$2.$3-$4');
        }

        return doc;
    };

    /**
     * cnpj
     * Method to generate random valid document number for Brazillian company
     *
     * @method pure.document.cnpj
     * @param {object} opts Options passed through object
     * @param {boolean= } [opts.format=false] Define that result is formated or not
     * @memberof pure.document
     * @returns {mixed} valid document number or object with valid document number
     */
    self.cnpj = (opts = {}) => {
        const def = opts || {};
        let doc = '';

        for (let i = 0; i < 8; i += 1) {
            doc += pure.random.number({ min: 0, max: 9 });
        }

        const mod = (str) => {
            const strlen = str.length;
            const num = str.substring(0, strlen);
            let sum = 0;
            let pos = strlen - 7;
            for (let i = strlen; i >= 1; i -= 1) {
                sum += num.charAt(strlen - i) * (pos -= 1);
                if (pos < 2) pos = 9;
            }
            return sum % 11 < 2 ? 0 : 11 - (sum % 11);
        };

        doc += '0001';
        doc += mod(doc);
        doc += mod(doc);

        if (def.format === true) {
            doc = doc.replace(/^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, '$1.$2.$3/$4-$5');
        }

        return doc;
    };
}

module.exports = Document;
